...
def get_ytd(ticker_symbol: str):
    try:
        ...
        close_col = "Close"
        s_ytd = float((stock[close_col].iloc[-1] - stock[close_col].iloc[0]) / stock[close_col].iloc[0] * 100)
        spy_ytd = float((spy[close_col].iloc[-1] - spy[close_col].iloc[0]) / spy[close_col].iloc[0] * 100)
...
def fetch_stock_data(ticker_symbol: str):
    ...
    # TTM revenue and net income from info first, fallback to income_stmt
...
async def get_summary(ticker: str):
    ...
    if not api_key:
        raise HTTPException(status_code=500, detail="ANTHROPIC_API_KEY לא מוגדר בקובץ .env")
